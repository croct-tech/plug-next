'use client';

import type {UseContentOptions, FetchResponse, VersionedSlotId} from '@croct/plug-react';
import {useContent as useContentReact} from '@croct/plug-react';
// eslint-disable-next-line import-x/extensions -- Extension needed to work on both CJS and ESM
import type {NextRouter} from 'next/router.js';
// eslint-disable-next-line import-x/extensions -- Extension needed to work on both CJS and ESM
import {useRouter as usePageRouter} from 'next/router.js';

export type {UseContentOptions, FetchResponse} from '@croct/plug-react';

function useContentNext(id: VersionedSlotId, options?: UseContentOptions<any, any>): FetchResponse<any, any> {
    const {preferredLocale, ...otherOptions} = options ?? {};
    const router = useRouter();
    const resolvedLocale = getLocale(preferredLocale ?? router.locale);

    return useContentReact<any, any, any>(
        id,
        resolvedLocale !== null
            ? {...otherOptions, preferredLocale: resolvedLocale}
            : otherOptions,
    );
}

function getLocale(locale?: string): string | null {
    if (locale !== undefined && locale !== '') {
        return locale;
    }

    const defaultLocale = process.env.NEXT_PUBLIC_CROCT_DEFAULT_PREFERRED_LOCALE ?? '';

    if (defaultLocale !== '') {
        return defaultLocale;
    }

    return null;
}

function useRouter(): Pick<NextRouter, 'locale'> {
    try {
        return usePageRouter();
    } catch {
        return {};
    }
}

export const useContent: typeof useContentReact = useContentNext;
