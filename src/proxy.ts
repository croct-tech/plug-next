...
type CroctProxyParams =
      [NextProxy]
    | [NextProxy, ProxyMatcher]
    | [CroctProxyOptions]
    | [];

export function withCroct(): NextProxy;
export function withCroct(next: NextProxy): NextProxy;

export function withCroct(next: NextProxy, matcher: ProxyMatcher): NextProxy;
export function withCroct(props: CroctProxyOptions): NextProxy;

export function withCroct(...args: CroctProxyParams): NextProxy {
    if (args.length === 0) {
        return withCroct({});
    }
    ...
    const matchesProxy = createMatcher((Array.isArray(matchers) ? matchers : [matchers]).flatMap(
        definition => (
